...
public class DesController implements Initializable {
    ...
    public TextField keyDES;
    public Button buttonEncDES;
    public TextArea finalTextDES;
    public TextField textDES;
    public Button buttonDecDES;
    ...
    public void buttonEncSSC(ActionEvent actionEvent) {

        int[] inputBits = setText();
        int[] keyBits = setKey();
        System.out.println("\n--- ENCRYPTION ---");
        int outputBits[] = permute(inputBits, keyBits, false);
        finalTextDES.setText(finalText(inputBits, keyBits, false));

    }

    public void buttonDecSSC(ActionEvent actionEvent) {
        ...
        finalTextDES.setText(finalText(inputBits, keyBits, true));
    }

    public int[] setText(){
        String text = textDES.getText();
        //pobieranie tekstu
        int inputBits[] = new int[64];
        for(int i=0 ; i < text.length() ; i++) {
            String s = Integer.toBinaryString(Integer.parseInt(Character.toString(text.charAt(i)), 16));
            while(s.length() < 4) {
                s = "0" + s;
            }
        ...
        //System.out.println(inputBits.length + " - inputBits length");
        return inputBits;
    }

    public int[] setKey(){
        String key = keyDES.getText();
    ...
    private static int[] permute(int[] inputBits, int[] keyBits, boolean isDecrypt) {
        ...
        for (int i = 0; i < newBits.length; i++) {
            System.out.print(newBits[i] + ",i=" + i +" || ");
        }
        ...
        //podzial 64bitowego bloku na dwa 32bitowe
    ...
    private static String finalText(int[] inputBits, int[] keyBits, boolean isDecrypt){
        int finalOutput[] = permute(inputBits, keyBits, isDecrypt);
        // Since the final output is stored as an int array of bits, we convert
        // it into a hex string:
